...
BatteryI2C = battery.BatteryManagementSystem()
RP2040i2c =  machine.I2C(0, sda=machine.Pin(24), scl=machine.Pin(25))
ChargerI2C = charger.CH224()
last_write_time = 0
while True:
    #wdt.feed()
    # Display battery level over USB (UART)
    if utime.time() - last_write_time >= 5:
        last_write_time = utime.time()
        try:
            # Assume a function get_battery_level() returns battery percentage (0-100)
            # 获取电池状态
            voltage = BatteryI2C.get_voltage()
            current = BatteryI2C.get_current()
            soc = BatteryI2C.get_state_of_charge()
            USB_protocal = ChargerI2C.get_protocol()
            USB_current = ChargerI2C.get_PD()
            
            #请你在这加一段代码，将voltage, current, soc, USB_protocal和USB_current都存入一个名叫running_log.txt的文件
            try:
                log_line = "{:0.0f}s | Voltage: {:.2f} V, Current: {:.2f} mA, SOC: {:.1f} %, USB_Protocol: {}, USB_Current: {} mA\n".format(
                    time.time(), voltage, current, soc, str(USB_protocal), USB_current
                )
                with open("running_log.txt", "a") as f:
                    f.write(log_line)
            except Exception as log_err:
                print("Failed to log to file:", log_err)
