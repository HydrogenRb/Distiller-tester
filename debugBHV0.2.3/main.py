...
def set_color(np, color, brightness=None, index=None):
    ...
    # Debug output to see actual values being set
    debug_print(f"[RP2040 DEBUG] Setting color: R={r}, G={g}, B={b}, brightness={np.brightness}\n")
...
def handle_neopixel_sequence(np, np2, data):
    ...
    for seq_num in sequence_numbers:
        ...
        try:
            ...
            if len(color_data) >= 5:
                ...
                with neopixel_lock:
                    # Set first neopixel (single LED)
                    set_color(np, [r, g, b], brightness, 0)
                    # Set second neopixel (all 4 LEDs)
                    set_color(np2, [r, g, b], brightness, None)
...
debug_print(f"[RP2040 DEBUG] Initialized {len(np)} NeoPixel on GPIO20\n")

# Initialize second neopixel with 4 LEDs
np2 = init_neopixel(pin=6, num_leds=4, brightness=0.5)
debug_print(f"[RP2040 DEBUG] Initialized {len(np2)} NeoPixel on GPIO6\n")

# Startup LED sequence: Red -> Green -> Blue -> Off
def startup_led_sequence():
    debug_print("[RP2040 DEBUG] Starting LED sequence: R-G-B-Off\n")
    colors = [
        ([255, 0, 0], 0.2, 500),    # Red, 50% brightness, 500ms
        ([0, 255, 0], 0.2, 500),    # Green, 50% brightness, 500ms  
        ([0, 0, 255], 0.2, 500),    # Blue, 100% brightness, 500ms
        ([0, 0, 0], 0.0, 500)       # Off, 0% brightness, 500ms
    ]
    
    for i, (color, brightness, delay) in enumerate(colors):
        debug_print(f"[RP2040 DEBUG] Step {i+1}: Setting RGB{color}, brightness: {brightness}\n")
        with neopixel_lock:
            # Set first neopixel (single LED)
            set_color(np, color, brightness, 0)
            # Set second neopixel (all 4 LEDs)
            set_color(np2, color, brightness, None)  # None means all LEDs
        debug_print(f"[RP2040 DEBUG] Step {i+1} completed, waiting {delay}ms\n")
        utime.sleep_ms(delay)
    
    debug_print("[RP2040 DEBUG] Startup LED sequence completed\n")
                        ...
                        try:
                            ...
                            if isinstance(data, dict):
                                ...
                                if function_type == 'NeoPixel':
                                    ...
                                    handle_neopixel_sequence(np, np2, data)  # Direct execution, no new thread
    ...
    if thread_handoff_complete:
        if debounce(upBTN) and selectBTN.value() == 1:
            ...
            if utime.ticks_diff(utime.ticks_ms(), start_time) >= 10000:
                einkStatus.low()  
                einkMux.loow() # SOM CONTROL E-INK
                uart0.write("xSAM_USB\n")
                if PRODUCTION:
                    switch_usb("SAM_USB")
                einkRunning = False
                machine.reset()  # 让RP2040重启
